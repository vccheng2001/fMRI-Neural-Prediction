...
def gaussian_naive_bayes(dataset):
    ...
    probs = calc_likelihoods(classes, data, priors, means, stdevs)
...
''' Step 2: Calculates mean, stdev for each feature'''
...
def calc_likelihoods(classes, data, priors, means, stdevs):
    probs = {}
...
''' For each row in training set, make class prediction '''
...
if __name__ == "__main__":
    dataset= np.genfromtxt("medium_data.csv", delimiter= ",", \
                                    skip_header=1, dtype='unicode')
    gaussian_naive_bayes(dataset)
